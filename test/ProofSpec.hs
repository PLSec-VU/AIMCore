...
proofTests =
  testGroup
    ...
    [ -- The base case, on the real 'Vec'-backed reset state. 'Proof.Functional.Induction.baseCase'
      ...
      -- A jump spliced into the memory stage. No reachable state has one -- a
      -- jump costs three cycles, so it has retired past writeback by the next
      -- aligned state -- and nothing else in the invariant rules it out, so this
      -- is the only thing constraining the @JType@ / @IType Jump@ clauses of
      -- 'Proof.Functional.Invariant.flushMeStage'. It fails if those clauses
      -- stop writing @rd@.
      ...
      testCase "fold-free invariant agrees with the list version" $
        let bad =
         ...
         in if P.null bad then pure () else assertFailure (show (P.take 5 bad)),
      -- The fifth counterexample of the earlier proof: a store whose byte range
      -- sits in (or wraps into) a PC word at the top of the address space. See
      -- the comment above 'wrapCESys'. That proof excluded this state by
      -- assumption; the core now handles it, so the state must survive its hop
      -- on its own. This is the regression test for the address-overlap fix.
      testCase "wrap-around aliasing store is handled by the core" $ do
        let sys = wrapCESys
            wr = 1
            wa = 0xFFFFFFFD
            isa = isaFromSys sys
            sys' = stepSysN (driver sys P.+ 1) sys
            isa' = case isaStep isa of Next x -> x; IsaHalted -> isa
        assertBool "state satisfies the invariant" (invAtFree wr wa isa sys)
        assertBool
          "decode stalls on the store hazard rather than taking a one-cycle hop"
          (driver sys P./= 0)
        assertBool
          "the driven hop preserves the invariant"
...
--
-- History, kept because it is what this state tests. The earlier proof excluded
-- aliasing stores by assumption, and that assumption first checked
--
...
-- which is false for EVERY store address -- so a store into that word passed
...
-- satisfying @p < a + n@.
--
-- There is no assumption now: 'Instruction.storeHazard' is wrap-correct and
-- 'Core.decode' stalls on the overlap, so this state has to survive its hop.
--
...
--     execute instruction.
--
-- The driver's @storeHazard/nomem@ branch used to be an empty proof case: the
-- old pre-state aliasing assumption rejected even a one-byte overlap with the
-- decode-stage word, so no state could reach it. With that assumption gone the
-- branch is reachable and wants generator coverage.
...
-- | States the driver sends on a four-cycle hop: a load-use hazard with the
-- incoming instruction, and memory instructions in all three older stages. The
-- store-hazard route into this case is reachable too, now that the aliasing
-- assumption is gone, and is not yet generated here.
